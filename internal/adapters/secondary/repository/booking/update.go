...
func (r *bookingRepository) Update(ctx context.Context, request domain.Booking) error {
	...
	_, err := db.QuerierFromContext(ctx, r.db).Exec(ctx, query, request.TrialClassID, request.StudentID, request.Status, request.IdempotencyKey, request.HoldExpiredAt, request.PaymentCode, request.ID)
	if err != nil {
